...
export default function UserNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      console.error("[UserNotifications] Error fetching notifications:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      ...
      console.error("[UserNotifications] Failed to mark as read:", error);
    }
  };
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      console.error("[UserNotifications] Failed to mark all as read:", error);
    }
  };
  ...
  const handleNotificationClick = (notification: Database["public"]["Tables"]["notifications"]["Row"]) => {
    if (notification.request_id) {
      if (notification.type === "new_message") {
        router.push(`/dashboard/messages/${notification.request_id}`);
      } else {
