...
export default function ResponderMessagesPage() {
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      // Fetch requests assigned to this responder that have messages
      const { data: requests, error: requestsError } = await supabase
        ...
        `)
        .eq("assigned_responder_id", (await supabase.auth.getUser()).data.user?.id)
        .order("created_at", { ascending: false });

      if (requestsError) {
        console.error("[ResponderMessages] Failed to fetch requests:", requestsError);
        return;
      }
      ...
      for (const request of requests || []) {
        ...
        if (messages && messages.length > 0) {
          ...
          conversationsData.push({
            request: request,
            otherParticipantName: profile?.full_name || "Patient",
      ...
      console.error("[ResponderMessages] Error fetching conversations:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConversationClick = (requestId: string) => {
    router.push(`/responder/messages/${requestId}`);
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div>
            ...
            <p className="text-slate-600">Communicate with patients about assigned emergency requests</p>
          </div>
        </motion.div>
