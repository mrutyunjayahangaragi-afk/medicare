...
export default function ResponderConversationPage({ params }: { params: { id: string } }) {
  ...
  const loadConversation = async () => {
    ...
    try {
      ...
      // Check if responder is assigned to this request
      if (requestData.assigned_responder_id !== user.id) {
      ...
      console.error("[ResponderConversation] Error loading conversation:", err);
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      console.error("[ResponderConversation] Failed to send message:", err);
      throw err;
    } finally {
      setIsSending(false);
    }
      ...
      <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <button
              onClick={() => router.push("/responder/messages")}
  ...
  const disabledReason = request.status === "cancelled"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Patient • {emergencyType?.label || "Emergency"}
                </p>
              </div>
            </div>
            {userProfile?.phone && (
